...
X1 <- model.matrix( ~ Age*SEX + Observer + Year, data)[,-1] # demographic predictors
P1 <- ncol(X1)
...
library(rstan)
fit <- stan(file = '~/code/OrdRegMix/mordreg.stan', data = param, iter = 10, warmup = 10, chains = 1, pars = c("u_raw", "u", "eta"), include = F)
# library(shinystan)
